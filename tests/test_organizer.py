...
class TestFileOrganizer:
    ...
    def create_test_file(self, directory: Path, filename: str, old: bool = False) -> Path:
        ...
        if old:
            ...
            file_path.touch()
            file_path.utime(times=(old_timestamp, old_timestamp))
    ...
    def test_init(self, temp_dir):
        ...
        assert organizer.old_files_days == 365
    ...
    def test_get_target_path(self, temp_dir, organizer):
        ...
        assert target == temp_dir / "Images" / "test.jpg"
    ...
    def test_get_target_path_conflict(self, temp_dir, organizer):
        ...
        assert target == images_dir / "test_1.jpg"
    ...
    def test_execute_with_errors(self, temp_dir, organizer):
        ...
        # Make target directory read-only (simulate permission error)
        target_dir = temp_dir / "Text"
        target_dir.mkdir()
        target_file = target_dir / "test.txt"
        target_file.touch()
        target_file.chmod(0o444)  # Read-only
        
        # On some systems, we might not be able to overwrite read-only files
        # This test might not work as expected on all platforms
        # but it demonstrates the error handling mechanism
    ...
    def test_extension_map_building(self, organizer):
        ...
        assert organizer.ext_to_category.get('.jpg') == organizer.ext_to_category.get('.JPG')
